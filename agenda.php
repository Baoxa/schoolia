                            ...
                            <?php
                              switch ($day) {
                                case 'lundi':
                                  ...
                                  $hour = $req->fetch();
                                  buttonPart($db,$hour['heure_debut_lundi'],$hour['heure_fin_lundi'],$profil['id'],$profil['type'],$day);
                                  break;
                                case 'mardi':
                                  $req = $db->query('SELECT heure_debut_mardi, heure_fin_mardi FROM cours WHERE id_tuteur='.$profil['id']);
                                  $hour = $req->fetch();
                                  buttonPart($db,$hour['heure_debut_mardi'],$hour['heure_fin_mardi'],$profil['id'],$profil['type'],$day);
                                  break;
                                case 'mercredi':
                                  $req = $db->query('SELECT heure_debut_mercredi, heure_fin_mercredi FROM cours WHERE id_tuteur='.$profil['id']);
                                  $hour = $req->fetch();
                                  buttonPart($db,$hour['heure_debut_mercredi'],$hour['heure_fin_mercredi'],$profil['id'],$profil['type'],$day);
                                  break;
                                case 'jeudi':
                                  $req = $db->query('SELECT heure_debut_jeudi, heure_fin_jeudi FROM cours WHERE id_tuteur='.$profil['id']);
                                  $hour = $req->fetch();
                                  buttonPart($db,$hour['heure_debut_jeudi'],$hour['heure_fin_jeudi'],$profil['id'],$profil['type'],$day);
                                  break;
                                case 'vendredi':
                                  $req = $db->query('SELECT heure_debut_vendredi, heure_fin_vendredi FROM cours WHERE id_tuteur='.$profil['id']);
                                  $hour = $req->fetch();
                                  buttonPart($db,$hour['heure_debut_vendredi'],$hour['heure_fin_vendredi'],$profil['id'],$profil['type'],$day);
                                  break;
                                case 'samedi':
                                  $req = $db->query('SELECT heure_debut_samedi, heure_fin_samedi FROM cours WHERE id_tuteur='.$profil['id']);
                                  $hour = $req->fetch();
                                  buttonPart($db,$hour['heure_debut_samedi'],$hour['heure_fin_samedi'],$profil['id'],$profil['type'],$day);
                                  break;
                                case 'dimanche':
                                  $req = $db->query('SELECT heure_debut_dimanche, heure_fin_dimanche FROM cours WHERE id_tuteur='.$profil['id']);
                                  $hour = $req->fetch();
                                  buttonPart($db,$hour['heure_debut_dimanche'],$hour['heure_fin_dimanche'],$profil['id'],$profil['type'],$day);
                                  break;
                              }
                            ?>
