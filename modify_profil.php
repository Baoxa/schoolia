    ...
    if (isset($_POST['id']) && !isset($_POST['id'])) {
      header("location: admin.php?message=Le profil n'a pas été modifié.");
      exit;
    }
    ...
    if(isset($_FILES['image']) && !empty($_FILES['image']['name'])) {
    	...
    	if(!in_array($_FILES['image']['type'], $acceptable)){
    		header('location: modify_profil.php?message=Format de fichier incorrect.&type=danger');
    		exit;
    	}
    	...
    	if($_FILES['image']['size'] > $maxSize){
    		header('location: modify_profil.php?message=Ce fichier est trop lourd !&type=danger');
    		exit;
    	}
    ...
    if(isset($_POST) AND !empty($_POST)){
      if(!empty($_POST['name']) AND !empty($_POST['firstname']) AND !empty($_POST['mail']) AND !empty($_POST['address']) AND !empty($_POST['postal']) AND !empty($_POST['city']) AND !empty($_POST['birthday'])){
        ...
        header('location:admin.php?message=Modification réussi.');
      }else{
        header('location:modify_profil.php?message=Profil non modifié.');
      }
    }
     ?>
